...
const buildSubjectsForScheme = (scheme: GradeScheme): Subject[] =>
  SUBJECT_CREDIT_TEMPLATES[scheme].map((credits, index) => ({
    ...
    name:
      scheme === '2018'
        ? (PHYSICS_2018_SUBJECTS[index] ?? `Subject ${index + 1}`)
        : scheme === '2021'
          ? (PHYSICS_2021_SUBJECTS[index] ?? `Subject ${index + 1}`)
          : scheme === '2022'
            ? (PHYSICS_2022_SUBJECTS[index] ?? `Subject ${index + 1}`)
        : `Subject ${index + 1}`,
    credits,
    marks: -1,
  }));
...
export default function SGPACalculator() {
  usePageTitle('Physics Cycle | Engineering Calculator');
      ...
      <main className="min-h-screen flex flex-col py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto w-full">
          <div className="mb-8">
            ...
            <h1 className="text-4xl font-bold ui-gradient-text mb-2"><ScrambleText text="Physics Cycle" /></h1>
            <p className="ui-muted">Calculate SGPA for Physics Cycle subjects by selected scheme</p>
          </div>
          ...
          <div className="ui-card p-8 ui-reveal" onKeyDown={handleEnterPress}>
            <div className="mt-2 mb-4">
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="inline-flex items-center text-xs font-bold text-cyan-300 hover:text-cyan-100 hover:underline underline-offset-4 transition-all duration-200 cursor-pointer hover:-translate-y-0.5"
                >
                  View Grade Scale
                </button>
              </div>
            ...
            <div className="mb-8">
                        ...
                        <div>
                          ...
                          {subject.marks >= 0 && derivedGrade && (
                            <div className="mt-2 h-12 flex items-center gap-2">
                              <span className="h-full px-3 rounded-lg border border-cyan-300/35 bg-cyan-500/20 text-cyan-100 text-sm font-bold ui-number whitespace-nowrap flex items-center">
              ...
              disabled={isResetting}
              className="w-full px-6 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded-lg hover:shadow-2xl hover:shadow-cyan-500/25 hover:brightness-110 hover:-translate-y-0.5 transition-all duration-200 font-semibold ui-interactive"
            >
              Calculate SGPA
            </button>

            {sgpa !== null && (
              <div ref={resultSectionRef} className="ui-result-card mt-8 p-6 bg-gradient-to-r from-blue-500/15 to-cyan-500/15 rounded-lg border border-cyan-300/35 ui-reveal ui-delay-1">
                ...
                <p className="ui-result-value text-4xl sm:text-5xl md:text-6xl !font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-300 ui-number">
                  {animatedSGPA.toFixed(2)}
                </p>
                <p className="ui-result-label text-slate-300 mt-6 mb-2">Converted Percentage ({percentageSchemeLabel})</p>
                <p className="ui-result-subvalue text-3xl sm:text-4xl md:text-5xl !font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-300 ui-number">
          ...
          >
            <div className="flex items-start justify-between gap-4 mb-5">
              <div className="flex items-start gap-3">
                <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-500/30 to-cyan-500/30 border border-cyan-300/30 flex items-center justify-center">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
              {Object.entries(
                ...
                <div key={letter} className="rounded-xl border border-white/10 bg-slate-900/55 hover:bg-slate-900/75 p-3.5 transition ui-interactive">
                  ...
                  <p className="text-cyan-300 font-bold text-lg ui-number">{info.value}</p>
