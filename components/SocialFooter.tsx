...
export default function SocialFooter({ showLegalLinks = false }: SocialFooterProps) {
  ...
  return (
    <footer className="text-slate-400 py-8 px-4 sm:px-6 lg:px-8 mt-auto w-full">
      <div className="max-w-7xl mx-auto ui-card p-5 sm:p-6 shadow-2xl drop-shadow-2xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-center lg:justify-end gap-2.5">
            {Connections.map((connection) => (
              <a
                ...
                className="w-10 h-10 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-slate-100 hover:bg-white/20 hover:border-blue-400/40 transition-all duration-200 ui-interactive hover:-translate-y-0.5"
              >
                <FontAwesomeIcon icon={connection.icon} className="w-4 h-4" />
              </a>
            ))}
          </div>
        </div>
